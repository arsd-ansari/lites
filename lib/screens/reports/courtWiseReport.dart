...
import 'package:lites/models/responses/reports/GetCourtWiseReportModel.dart';
...
class CourtWiseReport extends StatefulWidget {
  String routeName = '/CourtWiseReport';

  CourtWiseReport({super.key});

  @override
  State<CourtWiseReport> createState() => _CourtWiseReportState();
}

class _CourtWiseReportState extends State<CourtWiseReport> {
  ...
  List<String> columnTitles = ['Sr No', 'OIC', 'Appellant Name', 'Respondant Name', 'Court Name', 'Case Detail', 'Advocate Name'];
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      getAdmDepList();
      getCourtWiseDetails(page: currentPage);
    });
  }
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >=
      ...
      currentPage += 1;
      getCourtWiseDetails(page: currentPage, isPagination: true);
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: LitesAppBar(
        title: 'Court Wise Report Filter',
        onBackPressed: () {
                          ...
                          children: [
                            ElevatedButton(
                              onPressed: () {
                                ...
                                getCourtWiseDetails(page: 1, isPagination: false);
                              },
  ...
  void loadMore() async {
    ...
    await getCourtWiseDetails(page: nextPage, isPagination: true);
  ...
  Future getCourtWiseDetails({int page = 1, bool isPagination = false}) async {
    try {
      ...
      GetCourtWiseReqModel req = GetCourtWiseReqModel();
      ...
      if (isDefaultFilter) {
        ...
        req.courtId = selectedLevel?.value != null
      ...
      if (kDebugMode) {
        print('getCourtWiseDetails - raw request - ${jsonEncode(req.toJson())}');
      }
      final encryptedData = await EncryptionHelper.encryptData(req.toJson());
      final encryptedRequest = {"data": encryptedData};
      final encryptedResponse = await client.getCourtWiseReportDetails(encryptedRequest);

      final responseJson = jsonDecode(encryptedResponse);
      final decryptedMap =await EncryptionHelper.decryptData(responseJson["Data"]);

      if (kDebugMode) {
        print("Decrypted getCourtWiseDetails response: $decryptedMap");
      }

      final response = GetCourtWiseReportModel.fromJson( decryptedMap);
      ...
      if ((response.status ?? false) && response.data != null) {
        List<CourtWiseData> newCases = response.data!;
      ...
      if (kDebugMode) {
        print('getCourtWiseDetails - error - $e');
      }
      EssentialDialogs().showProgressHud(context, false);
      isLoadingMore = false;
