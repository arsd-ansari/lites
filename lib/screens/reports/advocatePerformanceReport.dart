...
import '../../models/responses/reports/GetAdvocatePerformanceReportModel.dart';
...
class AdvocatePerformanceReport extends StatefulWidget {
  String routeName = '/AdvocatePerformanceReport';

  AdvocatePerformanceReport({super.key});

  @override
  State<AdvocatePerformanceReport> createState() => _AdvocatePerformanceReportState();
}

class _AdvocatePerformanceReportState extends State<AdvocatePerformanceReport> {
  ...
  int currentPage = 1;
  int totalPages = 1;
  List<Data> groupAdvocateOptions = [];

  bool isLoadingMore = false;
  List<AdvocatePerfData> caseList = [];

  final List<Data> levelOptions = [
    Data(text: 'Supreme Court', value: '1'),
    Data(text: 'High Court Jodhpur', value: '5'),
    Data(text: 'High Court Bench Jaipur', value: '2'),
    Data(text: 'RCSAT', value: '4'),
    Data(text: 'District Court', value: '3'),
    Data(text: 'NATIONAL GREEN TRIBUNAL', value: '9'),
    Data(text: 'Tribunal Courts', value: '6'),
    Data(text: 'Other State High Court', value: '10'),
    Data(text: 'Other Than District Court', value: '13')
  ];
  List<String> columnTitles = ['Sr No', 'Advocate Name', 'Total Cases', 'Decided', 'Favour', 'Against', 'Percentage of cases in favour'];
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      getAdmDepList();
      getAdvocateList(0, 0);
      getAdvocatePerfDetails(page: currentPage);
    });
  }
  void _onScroll() {
    if (_scrollController.position.pixels >=
      ...
      currentPage += 1;
      getAdvocatePerfDetails(page: currentPage, isPagination: true);
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: LitesAppBar(
        title: 'Advocate Performance Summary Report Filter',
        onBackPressed: () {
                          ...
                          children: [
                            ElevatedButton(
                              onPressed: () {
                                ...
                                getAdvocatePerfDetails(page: 1, isPagination: false);
                              },
  ...
  void loadMore() async {
    ...
    await getAdvocatePerfDetails(page: nextPage, isPagination: true);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            '(Advocate Performance Summary Report )',
  ...
  Future getAdvocatePerfDetails({int page = 1, bool isPagination = false}) async {
    try {
      ...
      GetAdvocatePerformanceReqModel req = GetAdvocatePerformanceReqModel();
      ...
      bool isDefaultFilter = selectedAdminDept == null &&
          selectedHodUnitDept == null && selectedOffice == null && selectedLevel == null &&
          selectedFromDate == null &&
          selectedToDate == null;
      ...
      if (isDefaultFilter) {
        ...
        req.lawyerId = selectedOffice?.value != null
            ? int.parse(selectedOffice!.value.toString())
            : 0;
        req.courtTypeId = selectedLevel?.value != null
      ...
      if (kDebugMode) {
        print('getAdvocatePerfDetails - raw request - ${jsonEncode(req.toJson())}');
      }
      final encryptedData = await EncryptionHelper.encryptData(req.toJson());
      final encryptedRequest = {"data": encryptedData};
      final encryptedResponse = await client.getAdvocatePerformanceReportDetails(encryptedRequest);

      final responseJson = jsonDecode(encryptedResponse);
      final decryptedMap =await EncryptionHelper.decryptData(responseJson["Data"]);

      if (kDebugMode) {
        print("Decrypted getAdvocatePerfDetails response: $decryptedMap");
      }

      final response = GetAdvocatePerformanceReportModel.fromJson( decryptedMap);
      ...
      if ((response.status ?? false) && response.data != null) {
        List<AdvocatePerfData> newCases = response.data!;
        List<List<String>> newRows = newCases.map((caseItem) {  final decided = caseItem.decided ?? 0;
      ...
      if (kDebugMode) {
        print('getAdvocatePerfDetails - error - $e');
      }
      EssentialDialogs().showProgressHud(context, false);
      isLoadingMore = false;
  ...
  Future<void> getAdvocateList(adminDeptId, unitDeptId) async {
    try {
      EssentialDialogs().showProgressHud(context, true);

      groupAdvocateOptions = await commonRepository.getAdvocates(context,
        0,
        0,
