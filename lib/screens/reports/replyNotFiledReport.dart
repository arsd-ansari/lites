...
import '../../models/responses/reports/GetReplyNotFiledReportModel.dart';
...
class ReplyNotFiledReport extends StatefulWidget {
  String routeName = '/ReplyNotFiledReport';

  ReplyNotFiledReport({super.key});

  @override
  State<ReplyNotFiledReport> createState() => _ReplyNotFiledReportState();
}

class _ReplyNotFiledReportState extends State<ReplyNotFiledReport> {
  ...
  List<ReplyNotFiledData> caseList = [];

  List<String> columnTitles = ['Sr No', 'Office Name', 'Case No/Abbr/Year', 'Pett./Appellant Name', 'Respondant Name', 'Advocate Name', 'OIC name', 'Abbreviation', 'R E implication'];
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      getAdmDepList();
      getReplyNtFiledDetails(page: currentPage);
    });
  }
  void _onScroll() {
    if (_scrollController.position.pixels >=
      ...
      currentPage += 1;
      getReplyNtFiledDetails(page: currentPage, isPagination: true);
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: LitesAppBar(
        title: 'Reply Not Filed Report Filter',
        onBackPressed: () {
                          ...
                          children: [
                            ElevatedButton(
                              onPressed: () {
                                ...
                                getReplyNtFiledDetails(page: 1, isPagination: false);
                              },
  ...
  void loadMore() async {
    ...
    await getReplyNtFiledDetails(page: nextPage, isPagination: true);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            '(STATEMENT OF COURT WISE PENDING CASES-REPLY NOT FILLED CASES)',
  ...
  Future getReplyNtFiledDetails({int page = 1, bool isPagination = false}) async {
    try {
      ...
      GetReplyNotFiledReqModel req = GetReplyNotFiledReqModel();
      ...
      if (isDefaultFilter) {
        ...
        req.unitId = 0;
        req.officeId = 0;
        req.toDate = defaultFromDate;
        req.fromDate = defaultToDate;
      } else {
      ...
      if (kDebugMode) {
        print('getReplyNtFiledDetails - raw request - ${jsonEncode(req.toJson())}');
      }
      final encryptedData = await EncryptionHelper.encryptData(req.toJson());
      final encryptedRequest = {"data": encryptedData};
      final encryptedResponse = await client.getReplyNtFiledReportDetails(encryptedRequest);

      final responseJson = jsonDecode(encryptedResponse);
      final decryptedMap =await EncryptionHelper.decryptData(responseJson["Data"]);

      if (kDebugMode) {
        print("Decrypted getReplyNtFiledDetails response: $decryptedMap");
      }

      final response = GetReplyNotFiledReportModel.fromJson( decryptedMap);
      ...
      if ((response.status ?? false) && response.data != null) {
        List<ReplyNotFiledData> newCases = response.data!;
      ...
      if (kDebugMode) {
        print('getReplyNtFiledDetails - error - $e');
      }
      EssentialDialogs().showProgressHud(context, false);
      isLoadingMore = false;
